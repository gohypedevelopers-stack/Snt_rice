...
export async function GET(request: Request) {
  ...
  if (!tokenResponse.ok) return NextResponse.redirect(new URL("/login?error=google_failed", request.url));

  const tokenData = (await tokenResponse.json()) as { access_token?: string };
  if (!tokenData.access_token) return NextResponse.redirect(new URL("/login?error=google_failed", request.url));
  ...
  if (!profileResponse.ok) return NextResponse.redirect(new URL("/login?error=google_failed", request.url));
  const profile = (await profileResponse.json()) as { email?: string; name?: string };
  if (!profile.email) return NextResponse.redirect(new URL("/login?error=google_failed", request.url));
